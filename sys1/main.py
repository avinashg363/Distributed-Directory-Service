...
li = []
li.append(4)
replicated_root.name = 'HALLS'
replicated_root.parent = None
replicated_root.tablename = None
replicated_root.classname = 'InstitutionalBody'
replicated_dic[4] = replicated_root
replicated_invdic[replicated_root] = 4
...
def updateFunction(final_values,deleted_var):
	...
	for var in deleted_var:
		if(var[1][0]=="'"):
			...
			_str=var[1]
		query = 'delete from '+var[0]+' where ID='+"'"+_str+"'"+';'
		# print query
...
print 'Enter system 2 ip:'
ip = raw_input()
sys1 = (ip,'IITKGP,KGPCAMPUS')
print 'system 2 ip is :', ip

print 'Enter system 2 port:'
port1 = raw_input()
sysport1 = (port1,'IITKGP,KGPCAMPUS')
print 'system 2 port is :', port1


print 'Enter system 3 ip:'
ip = raw_input()
sys2 = (ip,'IITKGP,KGPCAMPUS,HALLS')
print 'system 3 ip is :', ip


print 'Enter system 3 port:'
port2 = raw_input()
sysport2 = (port2,'IITKGP,KGPCAMPUS,HALLS')
print 'system 3 port is :', port2
...
while(True):
	...
	try:
		...
		if com[0] == 'subtreesearch':
			if len(coms) > 2:
				if syst == 2 or syst == 3:
					print 'Requesting to: ',sys2[0] 
					retstring = 'REFERRAL ' + str(sys2[0]) + ' ' + str(sysport2[0])
					c.send(retstring)
					c.close()
					continue
				else:
				...
				print 'Requesting to: ',sys2[0] 
				retstring = 'SUBTREEREFERRAL ' + str(sys2[0]) + ' ' + str(sysport2[0])
				c.send(retstring)
				c.close()
				continue
		if com[0] == 'freeze':
			...
			merge = open('mergeFile_received.txt',"w")
			...
			logfile2.close()
			logfile1.close()
			finalvalue,deletedvar = fonal.finalvalue('mergeFile_received.txt','initial_table_2.txt','initial_table_2_received.txt')
			...
			os.remove('mergeFile_received.txt')
